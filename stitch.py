...
def stitching (chunk_1 : np.array, chunk_2 : np.array, coord_chunk_1 : tuple, coord_chunk_2 :tuple, overlap:int, chunk_size:int, tile_size:int)->np.array:
    plt.figure()
    plt.subplot(1,2,1)
    ...
    unique_labels1 = unique_labels1[unique_labels1 != 0]  # optionnel : ignorer l'arrière-plan
    ...
    print(f"nombre de poly dans chunk_1 {len(polys1)}) et nb de centroides : {len(centros1)}")
    ...
    unique_labels2 = unique_labels2[unique_labels2 != 0]  # optionnel : ignorer l'arrière-plan
    ...
    print(f"nombre de poly dans chunk_2 {len(polys2)}) et nb de centroides : {len(centros2)}")
    ...
    image_full[:, w1-overlap*tile_size:] = chunk_2_relabel[:,overlap*tile_size:]
...
def process_mask(src, label, smooth, convex_hull, offset, x_offset=0, y_offset=0, return_centroid=False):
    ...
    if return_centroid:
        ...
        centroid = (int(round(moments["m10"] / m00)) + x_offset,
