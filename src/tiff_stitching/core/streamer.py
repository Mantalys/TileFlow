...
class Streamer:
    ...
    def _build_tiles(self):
        ...
        for y in ys:
            for x in xs:
                ...
                pad_right = self.config.tile_size - (x1 - x) if (x1 - x) < self.config.tile_size else 0
                pad_bottom = self.config.tile_size - (y1 - y) if (y1 - y) < self.config.tile_size else 0
    ...
    def _build_reconstructed(self):
        self._reconstructed = [
            np.zeros((self._shape[0], self._shape[1]), dtype=np.float32) for _ in range(self.config.n_features)
        ]
    ...
    def batchify(self, batch_size: int, scale: float = 1.0):
        ...
        def tile_generator():
            for i in range(0, len(self._tiles), batch_size):
                ...
                tiles = [self._data[tile["bbox"][1]:tile["bbox"][3], tile["bbox"][0]:tile["bbox"][2]] for tile in tiles]
                ...
                for j in range(len(tiles)):
                    ...
                    if tile.shape[0] < self.config.tile_size or tile.shape[1] < self.config.tile_size:
                        ...
                        tile = np.pad(tile, ((0, pad_height), (0, pad_width)), mode='constant', constant_values=0)
                    if scale != 1.0:
                        # Resize the tile if scale is not 1.0
                        tile = cv2.resize(tile, (self.config.tile_size * scale, self.config.tile_size * scale), interpolation=cv2.INTER_CUBIC)
                    tiles[j] = tile
                stacked = np.stack(tiles, axis=0)
                # add channel dimension at the end
                stacked = stacked[..., np.newaxis]  # shape: (batch_size, tile_size, tile_size, 1)
                yield stacked

        return tile_generator()
    
    def stamp_features(self, tile_index: int, features: list):
        assert len(features) == self.config.n_features, "Features length must match n_features in config"
        for i in range(len(features[0])):
            ...
            for feature_index in range(self.config.n_features):
                ...
                stamp(self._reconstructed[feature_index], unpadded, tile["core_bbox"], (0, 0))
            tile_index += 1
        return tile_index
